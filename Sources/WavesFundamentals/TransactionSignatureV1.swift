...
public extension TransactionSignatureV1 {
    ...
    enum Structure {
        ...
        public struct Sponsorship {
            ...
            public init(fee: Int64, chainId: Int, senderPublicKey: String, timestamp: Int64, assetId: String, minSponsoredAssetFee: Int64?) {
                self.fee = fee
        ...
        public struct SetScript {
            ...
            public init(fee: Int64, chainId: Int, senderPublicKey: String, timestamp: Int64, script: Base64?) {
                self.fee = fee
                self.chainId = chainId
                self.senderPublicKey = senderPublicKey
                self.timestamp = timestamp
        ...
        public struct SetAssetScript {
            ...
            public init(fee: Int64, chainId: Int, senderPublicKey: String, timestamp: Int64, assetId: String, script: Base64?) {
                self.fee = fee
                self.chainId = chainId
                self.assetId = assetId
                self.senderPublicKey = senderPublicKey
        ...
        public struct MassTransfer {
            ...
            public init(fee: Int64, chainId: Int, senderPublicKey: String, timestamp: Int64, assetId: String, attachment: String, transfers: [Transfer]) {
                self.fee = fee
                self.chainId = chainId
                self.senderPublicKey = senderPublicKey
                self.timestamp = timestamp
        ...
        public struct Data {
            ...
            public init(fee: Int64, data: [Value], chainId: Int, senderPublicKey: String, timestamp: Int64) {
                self.fee = fee
                self.data = data
                self.chainId = chainId
                self.senderPublicKey = senderPublicKey
...
private extension TransactionSignatureV1.Structure.MassTransfer {
    ...
    func transfersBytes(version: Int, chainId: Int) -> [UInt8] {
        var bytes: [UInt8] = []
...
private extension TransactionSignatureV1.Structure.InvokeScript.Call {
    ...
    var argsBytes: [UInt8] {
        ...
        bytes += getBytes(args: self.args)
        bytes = toByteArray(Int32(args.count)) + bytes
        
        return bytes
    }
    
    func getBytes(args: [TransactionSignatureV1.Structure.InvokeScript.Arg]) -> [UInt8] {
        var bytes: [UInt8] = []
        ...
        for arg in args {
            ...
            case .list(let value):
                ...
                let newBytes = value.map { listItem in
                    switch listItem.value.self {
                    case .bool(let val):
                        return TransactionSignatureV1.Structure.InvokeScript.Arg(value: .bool(val))
                    case .integer(let val):
                        return TransactionSignatureV1.Structure.InvokeScript.Arg(value: .integer(val))
                    case .string(let val):
                        return TransactionSignatureV1.Structure.InvokeScript.Arg(value: .string(val))
                    case .binary(let val):
                        return TransactionSignatureV1.Structure.InvokeScript.Arg(value: .binary(val))
                    case .list(_):
                        return TransactionSignatureV1.Structure.InvokeScript.Arg(value:.bool(false))
                    }
                }
                
                bytes += getBytes(args: newBytes)
            }
        }
